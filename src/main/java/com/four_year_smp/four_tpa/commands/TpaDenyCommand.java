...
public class TpaDenyCommand extends AbstractTpaCommand implements TabCompleter {
    public TpaDenyCommand(LocalizationHandler localizationHandler, ITeleportManager teleportManager) {
    ...
    public boolean onCommand(CommandSender invoker, Command command, String label, String[] args) {
        ...
        TeleportRequest request = _teleportManager.getRequest(target.getUniqueId(), sender.getUniqueId());
        if (request == null) {
            // You have no pending TPA requests.
            target.sendMessage(_localizationHandler.getTpaReceiverNotFound(sender.getName()));
            return true;
        }
